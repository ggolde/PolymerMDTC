...
def polymerize_rw(mol, n, headhead=False, confId=0, tacticity='atactic', atac_ratio=0.5, tac_array=None, check_tac_overide = False,
...
def amorphous_mixture_cell(mols, n, cell=None, density=0.03, retry=10, retry_step=100, threshold=2.0, dec_rate=0.8, check_structure=True):
    """
        ...
        for i in tqdm(range(n[m]), desc='[Unit cell generation %i/%i]' % (m+1, len(mols)), disable=const.tqdm_disable):
            for r in range(retry_step):
                ...
                if not check_structure:
                    break
                elif check_3d_structure_cell(cell_c, mol_coord, dist_min=threshold):
                    break
                elif r < retry_step-1:
                    utils.radon_print('Retry placing replicate in cell')
                else:
...
def check_tacticity(mol, tacticity, tac_array=None, confId=0):
    ...
    elif tacticity == 'atactic' or tacticity == 'manual':
        if tac_array is None:
            check = True
